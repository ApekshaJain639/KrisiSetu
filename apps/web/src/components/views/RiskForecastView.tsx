...
export const RiskForecastView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {/* ── FarmDesk Weather Advisory Header ── */}
      <div className="pb-2 border-b border-slate-200 dark:border-krishi-darkborder flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="self-start sm:self-auto flex items-center gap-2 px-3.5 py-2 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700 rounded-xl text-xs font-bold transition shadow-sm hover:bg-emerald-100"
        >
          {isGpsLocating ? (
          ...
          <span>{gpsStatusMessage || `${location} (${latitude?.toFixed(4)}°N, ${longitude?.toFixed(4)}°E)`}</span>
        </button>
      </div>
